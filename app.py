...
app.config['SQLALCHEMY_DATABASE_URI'] = f'postgresql://postgres:{pw}@localhost/warbler'
...
def homepage():
    ...
    if g.user:
        ...
        #     for message in following.message:
        #         message = (Message
        #                     .query
        #                     .order_by(Message.timestamp.desc())
        #                     .limit(100)
        ...
        followers_id = [follower.id for follower in g.user.following]
        # messages = Message.query.filter(followers_id).order_by(
        #     Message.timestamp.desc()).limit(100).all()
        messages = [m for m in db.session.query(Message).filter(Message.user_id.in_(followers_id)).order_by(
            Message.timestamp.desc()).limit(100).all()]
